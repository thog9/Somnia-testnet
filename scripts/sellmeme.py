...
# Hàm approve token
async def approve_token(w3: Web3, private_key: str, token_address: str, spender_address: str, amount: float, wallet_index: int, token_symbol: str, language: str = 'en'):
...
# Hàm bán token
async def sell_token(w3: Web3, private_key: str, token_symbol: str, amount: float, wallet_index: int, language: str = 'en'):
    account = Account.from_key(private_key)
...
# Hàm chính
async def run_sellmeme(language: str = 'en'):
    print()
    ...
    for i, (profile_num, private_key) in enumerate(private_keys, 1):
        ...
        approve_tx = await approve_token(w3, private_key, TOKENS[token_symbol]["address"], SPENDER_ADDRESS, amount, i, token_symbol, language)
        if not approve_tx:
            print_separator()
            continue
        
        if await sell_token(w3, private_key, token_symbol, amount, i, language):
            successful_sells += 1
