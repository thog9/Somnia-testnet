...
SOMNI_CONTRACT = "0xfA139F427a667b56d93946c2FD2c03601BaD033A"
...
LANG = {
    'vi': {
        'title': 'MINT SOMNI ✨ - SOMNIA TESTNET',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'no_balance': 'Số dư ví không đủ (< {required:.6f} ETH), không thể mint',
        'preparing_tx': 'Chuẩn bị giao dịch...',
        'sending_tx': 'Đang gửi giao dịch...',
        'success': 'Mint Somni ✨ thành công!',
        'failure': 'Mint Somni ✨ thất bại',
    ...
    'en': {
        ...
        'no_balance': 'Insufficient wallet balance (< {required:.6f} STT), cannot mint',
        'preparing_tx': 'Preparing transaction...',
        'sending_tx': 'Sending transaction...',
        'success': 'Successfully minted Somni ✨!',
        'failure': 'Failed to mint Somni ✨',
...
# Hàm mint Somni NFT
async def mint_somni_nft(w3: Web3, private_key: str, wallet_index: int, language: str = 'en'):
    account = Account.from_key(private_key)
    sender_address = account.address

    print(f"{Fore.CYAN}  > {LANG[language]['checking_balance']}{Style.RESET_ALL}")
    nft_contract = w3.eth.contract(address=Web3.to_checksum_address(SOMNI_CONTRACT), abi=NFT_ABI)
    try:
        ...
        print(f"{Fore.RED}  ✖ Lỗi kiểm tra số dư NFT: {str(e)}{Style.RESET_ALL}")
        return False
    ...
    mint_price = w3.to_wei(0, 'ether')  

    # Tạo dữ liệu giao dịch dựa trên sender_address
    data = (
        "0x84bb1e42"
    ...
    gas_price = max(base_fee + w3.to_wei('1', 'gwei'), w3.to_wei('5', 'gwei'))

    tx = {
        'from': sender_address,
        'to': Web3.to_checksum_address(SOMNI_CONTRACT),
        'value': mint_price,
        'data': data,
    ...
    required_balance = w3.from_wei(tx['gas'] * gas_price + mint_price, 'ether')
    ...
    try:
        ...
        print(f"{Fore.YELLOW}    {'Tx Hash':<16}: {tx_link}{Style.RESET_ALL}")
        return True  # Giả định thành công nếu timeout
...
async def run_mintaura(language: str = 'en'):
    ...
    for i, (profile_num, private_key) in enumerate(private_keys, 1):
        ...
        if await mint_somni_nft(w3, private_key, profile_num, language):
            successful_mints += 1
