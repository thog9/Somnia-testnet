...
def is_valid_private_key(key: str) -> bool:
    ...
    try:
        ...
        return len(key) == 66  # Độ dài private key hợp lệ (64 ký tự + '0x')
    except ValueError:
        return False
...
# Hàm tạo bytecode mint (tương tự bytecodeMintPongPing)
...
async def mint_pongping(web3: Web3, private_key: str, wallet_index: int, language: str) -> bool:
    try:
        ...
        # Thông tin hợp đồng
        CONTRACT_ADDRESS = "0x7968ac15a72629e05f41b8271e4e7292e0cc9f90"  # Từ cmdMintPong.ts
        ...
        if receipt.status == 1:
            print(f"{Fore.GREEN}  ✔ {'Thành công' if language == 'vi' else 'Success'}: Ví: {wallet_index} │ {'Đã mint 1000 $PONG' if language == 'vi' else 'Minted 1000 $PONG successfully'}{Style.RESET_ALL}")
            return True
        else:
...
# Hàm chính chạy mintpong
async def run_mintpong(language: str):
    # Thông báo bắt đầu với khung
    print_border(f"{'BẮT ĐẦU MINT $PONG' if language == 'vi' else 'START MINTING $PONG'}", Fore.CYAN)
    
    # Tải private keys từ pvkey.txt
...
if __name__ == "__main__":
    asyncio.run(run_mintpong('en'))  # Ngôn ngữ mặc định là English nếu chạy trực tiếp
