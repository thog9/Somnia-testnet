...
PONG_CONTRACT = Web3.to_checksum_address("0x9beaA0016c22B646Ac311Ab171270B0ECf23098F")
...
# Token information
TOKENS = [
    {"symbol": "PONG", "address": PONG_CONTRACT, "amount": 1000, "decimals": 18}
]
...
LANG = {
    'vi': {
        'title': '✨ MINT $PONG - SOMNIA TESTNET ✨',
        'info': 'ℹ Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'already_minted': '❌ Ví này đã mint $PONG rồi! Vui lòng không thử lại.',
    ...
    'en': {
        ...
        'already_minted': '❌ Wallet already minted $PONG! Please do not try again.',
...
def display_all_wallets_balances(w3: Web3, private_keys: list, language: str = 'en'):
    ...
    print(f"{Fore.CYAN}  Wallet | {'STT':<10} | {'PONG':<10}{Style.RESET_ALL}")
    ...
    for i, (profile_num, key) in enumerate(private_keys, 1):
        ...
        pong_balance = check_balance(w3, address, TOKENS[0]["address"], TOKENS[0]["decimals"], language)
        print(f"{Fore.YELLOW}  {i:<6} | {eth_balance:>10.6f} | {pong_balance:>10.6f}{Style.RESET_ALL}")
...
def check_balance(w3: Web3, address: str, token_address: str, decimals: int, language: str = 'en') -> float:
    if token_address == "native":
        ...
        contract = w3.eth.contract(address=Web3.to_checksum_address(token_address), abi=PONG_ABI)
        try:
            balance = contract.functions.balanceOf(address).call()
            return balance / (10 ** decimals)
        except Exception as e:
            print(f"{Fore.YELLOW}  ⚠ {LANG[language]['balance_check_failed'].format(symbol='PONG')}: {str(e)}{Style.RESET_ALL}")
            return -1
...
async def mint_token(w3: Web3, private_key: str, wallet_index: int, token: dict, proxy: str = None, language: str = 'en'):
    ...
    contract = w3.eth.contract(address=Web3.to_checksum_address(token['address']), abi=PONG_ABI)
...
async def run_mintpong(language: str = 'en'):
    print()
