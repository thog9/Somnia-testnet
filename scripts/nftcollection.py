...
# Constants
NETWORK_URLS = ['https://dream-rpc.somnia.network']
CHAIN_ID = 50312
...
LANG = {
    'vi': {
        'title': 'QUẢN LÝ NFT - SOMNIA TESTNET',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
    ...
    'en': {
        'title': 'NFT MANAGEMENT - SOMNIA TESTNET',
        'info': 'Info',
        'found': 'Found',
        'wallets': 'wallets',
        ...
        'no_balance': 'Insufficient balance (need at least {required} STT)',
...
async def deploy_nft(w3: Web3, private_key: str, wallet_index: int, name: str, symbol: str, max_supply: int, language: str = 'en'):
    ...
    try:
        ...
        # Lấy baseFee từ khối mới nhất
        latest_block = w3.eth.get_block('latest')
        base_fee = latest_block['baseFeePerGas']
        priority_fee = w3.to_wei('1', 'gwei')  # Tip 1 Gwei
        max_fee = base_fee + priority_fee  # Tổng phí tối đa
        ...
        required_balance = w3.from_wei(tx['gas'] * max_fee, 'ether')
...
async def run_nftcollection(language: str = 'en'):
    ...
    if choice == '1':  # Deploy
        ...
        symbol = input(f"{Fore.YELLOW}  > {LANG[language]['enter_symbol']}{Style.RESET_ALL}").strip() or "STT"
