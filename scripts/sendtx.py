...
LANG = {
    'vi': {
        'title': '✨ GỬI GIAO DỊCH - SOMNIA TESTNET ✨',
        'info': 'ℹ Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
    ...
    'en': {
        'title': '✨ SEND TRANSACTION - SOMNIA TESTNET ✨',
        'info': 'ℹ Info',
        'found': 'Found',
        'wallets': 'wallets',
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            if balance < required_balance:
                print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=required_balance)}: {balance:.6f} STT{Style.RESET_ALL}")
                return False
