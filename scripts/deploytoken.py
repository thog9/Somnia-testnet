...
LANG = {
    'vi': {
        'title': '✨ TRIỂN KHAI TOKEN ERC20 - SOMNIA TESTNET ✨',
        'info': 'ℹ Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            if eth_balance < CONFIG['MINIMUM_BALANCE']:
                print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=CONFIG['MINIMUM_BALANCE'])}: {eth_balance:.6f} STT{Style.RESET_ALL}")
                return None
            ...
            if eth_balance < required_balance:
                print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=required_balance)}: {eth_balance:.6f} STT{Style.RESET_ALL}")
                return None
            ...
            if receipt is None:
                ...
                print(f"{Fore.YELLOW}    - {LANG[language]['balance']}: {w3.from_wei(w3.eth.get_balance(sender_address), 'ether'):.6f} STT{Style.RESET_ALL}")
